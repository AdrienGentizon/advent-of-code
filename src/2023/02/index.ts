export const inputs = `Game 1: 7 blue, 5 red; 10 red, 7 blue; 5 blue, 4 green, 15 red; 4 green, 6 red, 7 blue; 5 green, 8 blue, 4 red; 5 red, 4 blue, 3 green
...
type GameWithRule = Game & { rule: Rule; power: number }

// return string as ['0red', '0green', '0blue']
function getSetsAsStringFromGameAsString(gameAsString: string): string[] {
  return gameAsString.split(':')[1].replaceAll(' ', '').split(';')
}

// reduce ['0red', '0green', '0blue'] to {red: 0, green: 0, blue: 0}
function reduceColorResultsStringToSet(rule: Rule) {
  return function (prev: Set, colorResultAsString: string): Set {
    switch (true) {
      case colorResultAsString.includes('red'):
        const redCubes = parseInt(colorResultAsString.replace('red', ''))
        return {
          ...prev,
          red: redCubes,
          isValid: !prev.isValid ? false : redCubes <= rule.red,
        }
      case colorResultAsString.includes('green'):
        const greenCubes = parseInt(colorResultAsString.replace('green', ''))
        return {
          ...prev,
          green: greenCubes,
          isValid: !prev.isValid ? false : greenCubes <= rule.green,
        }

      case colorResultAsString.includes('blue'):
        const blueCubes = parseInt(colorResultAsString.replace('blue', ''))
        return {
          ...prev,
          blue: blueCubes,
          isValid: !prev.isValid ? false : blueCubes <= rule.blue,
        }
...
// convert Game 1: 7 blue, 5 red; 10 red, 7 blue;
// to Game object with an id
// {
//   id: 1,
//   sets: [
//     { red: 5, green: 0, blue: 7 },
//     { red: 10, green: 0, blue: 7 },
//   ],
// }
function getGameWithSetsFromString(gameAsString: string): Game {
  const id = parseInt(gameAsString.split(':')[0].split(' ')[1])
  const sets = getSetsAsStringFromGameAsString(gameAsString).reduce(
    (prev: Set[], curr) => {
      const set = curr
        .split(',')
        .reduce(
          reduceColorResultsStringToSet({ red: 12, green: 13, blue: 14 }),
          { red: 0, green: 0, blue: 0, isValid: true }
        )
      return [...prev, set]
    },
    []
  ...
  return { id, sets, isValid: !sets.some(({ isValid }) => !isValid) }
}

export function compute() {
  const games: Game[] = []
  for (const input of inputs.split('\n')) {
    games.push(getGameWithSetsFromString(input))
  }
  const validGames: Game[] = []
  const augmentedGames: GameWithRule[] = []
  for (const game of games) {
    augmentedGames.push({
      ...game,
      rule: {
        red: game.sets.reduce(
          (max, curr) => (curr.red > max ? curr.red : max),
          0
        ),
        green: game.sets.reduce(
          (max, curr) => (curr.green > max ? curr.green : max),
          0
        ),
        blue: game.sets.reduce(
          (max, curr) => (curr.blue > max ? curr.blue : max),
          0
        ),
      },
      power: 0,
  ...
  return {
    partOne: validGames.reduce((sumOfIds, curr) => {
      return (sumOfIds += curr.id)
    }, 0),
    partTwo: augmentedGames
      .map((game) => ({
        ...game,
        power: game.rule.red * game.rule.green * game.rule.blue,
      }))
      .reduce((power, curr) => power + curr.power, 0),
  }
}
