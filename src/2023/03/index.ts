...
function mapCellFromRegexMatch(n: number) {
  return function (arr: RegExpMatchArray): CellNumber {
    const width = arr.at(0)?.length ?? 0
    const x = arr.index ?? 0
    const y = n
    const topLeft = { x: x - 1, y: y - 1 }
    const bottomRight = { x: x + width, y: y + 1 }
    return {
      value: parseInt(arr.at(0) ?? '0'),
      width,
      x,
      y,
      topLeft,
...
function getNumbers(inputs: string[]): Row[] {
  return inputs.map((input, n) => {
    return {
      index: n,
      value: input,
      numbers: [...input.matchAll(patternOnlyNumbers)].map(
        mapCellFromRegexMatch(n)
      ),
    }
  })
}

export function compute(inputs: string) {
  const rows = getNumbers(inputs.split(patternNewLine))
  const numbersAsEnginePart: (CellNumber & { isEnginePart: Boolean })[] = []
  const CHAR_IF_INDEX_OUT_OF_INPUT_EDGES = '?'

  for (const row of rows) {
    row.numbers.forEach((number) => {
      const previousRow =
        number.y - 1 < 0
          ? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
          : (
              rows.at(number.y - 1)?.value ?? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
            ).substring(number.topLeft.x, number.bottomRight.x + 1)
      const nextRow =
        number.y + 1 >= rows.length
          ? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
          : (
              rows.at(number.y + 1)?.value ?? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
            ).substring(number.topLeft.x, number.bottomRight.x + 1)
      const leftCell =
        number.topLeft.x < 0
          ? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
          : row.value.at(number.topLeft.x) ?? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
      const rightCell =
        number.bottomRight.x > row.value.length
          ? CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
          : row.value.at(number.bottomRight.x) ??
            CHAR_IF_INDEX_OUT_OF_INPUT_EDGES
      const isEnginePart =
        [
          ...`${previousRow}${rightCell}${nextRow}${leftCell}`.matchAll(
            patternSpecialCharacters
          ),
        ].length > 0

      numbersAsEnginePart.push({ ...number, isEnginePart })
    })
  }
  return {
    title: '03/12',
    partOne: numbersAsEnginePart
      .filter(({ isEnginePart }) => isEnginePart)
      .reduce((sum, curr) => (sum += curr.value), 0),
    partTwo: undefined,
  }
}
logResults(compute(inputs)) // 533784
